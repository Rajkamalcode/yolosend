...
def process_pdf_file(pdf_path, client, model, prompt):
    ...
    try:
        ...
        if len(all_pages_images) > 1:
            ...
            return generate_content_with_image(client, model, prompt, all_pages_images[0]["data"], all_pages_images[0]["mime_type"])
        else:
...
def create_single_doc_extraction_prompt_v2(filename):
    """Creates a heavily revised prompt for the LLM with detailed instructions for the 7 high-priority fields. Adjusted for file-based analysis."""
    prompt = f"""You are an expert Legal Document Analyst AI. Your task is to perform a high-precision extraction from the provided document file '{filename}'. You will help the Credit manager by creating a proper extraction not missing any items. You must generate a single JSON object as output.
...
def render_single_doc_view():
    ...
    if uploaded_file and st.button("🔎 Analyze Document", use_container_width=True):
        ...
        with st.status("Processing...", expanded=True) as status:
            ...
            try:
                # First Gemini call: Extract data AND generate guidance questions
                status.update(label="Performing comprehensive analysis with AI...")
                prompt = create_single_doc_extraction_prompt_v2(uploaded_file.name)
                response = process_document(temp_path, prompt)
                with open("extracted_response.txt", "w", encoding="utf-8") as f:
                    f.write(str(response))                # Clean up temp file
                try:
                    with open("extracted_response.json", "w", encoding="utf-8") as f:
                ...
                if "structured_data" in response and response["structured_data"]:
                    ...
                    extracted_data = parse_single_doc_extraction_response_v2(llm_response, uploaded_file.name, "")
                    st.session_state.single_doc_result_v2 = extracted_data
                
                # Debug logging
                print(f"Extracted data keys: {extracted_data.keys() if isinstance(extracted_data, dict) else 'Not a dict'}")
                
                # Only check for error after storing the result
                ...
                if RAG_AVAILABLE and st.session_state.get("enable_rag", True):
                    ...
                    try:
                        ...
                        if all_lookups:
                            # Second Gemini call: Generate final grounded opinion
                        ...
                        import traceback
                        traceback.print_exc()
                        # Continue even if RAG fails - don't return
    ...
    if st.session_state.single_doc_result_v2:
        ...
        with st.expander("Property Summary", expanded=True):
            ...
            cols[0].info(result.get("PROPERTY ADDRESS", "Not Found"))
            cols[1].write("**Owner(s):**")
            cols[1].info(result.get("OWNER NAME(S)", "Not Found"))
            ...
            section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "boundaries"]
            if section_issues:
        ...
        with st.expander("Documents Examined", expanded=False):
            ...
            # Discrepancies for this section
            section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "documents_examined"]
            if section_issues:
                st.subheader("Discrepancies")
                df_discrepancies = pd.DataFrame(section_issues)
                st.dataframe(df_discrepancies, use_container_width=True, hide_index=True)
        ...
        with st.expander("📜 Title History", expanded=True):
            ...
            if isinstance(title_history, list) and title_history:
                ...
                for entry in title_history:
                    with st.container(border=True):
                        ...
                        section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "property_nature"]
            ...
            # Discrepancies for this section
            section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "title_history"]
            if section_issues:
                st.subheader("Discrepancies")
                df_discrepancies = pd.DataFrame(section_issues)
                st.dataframe(df_discrepancies, use_container_width=True, hide_index=True)
        ...
        with st.expander("Mortgage Documents", expanded=False):
            ...
            # Get all documents from both categories
            pre_disbursal = mortgage_docs.get("pre_disbursal", [])
            post_disbursal = mortgage_docs.get("post_disbursal", [])
            
            # If no documents found in either category
            if not pre_disbursal and not post_disbursal:
                st.warning("No mortgage documents found in the document.")
            else:
                # Display all documents in a single table if no post-disbursal documents
                if not post_disbursal:
                    st.write("**Required Documents**")
                    if pre_disbursal:
                        df_docs = pd.DataFrame(pre_disbursal)
                        if "document_name" in df_docs.columns:
                            df_docs = df_docs.rename(columns={"document_name": "Document Name", "document_description": "Document Description"})
                            # Reorder columns
                            if "Document Description" in df_docs.columns:
                                df_docs = df_docs[["Document Name", "Document Description"]]
                        st.dataframe(df_docs, use_container_width=True, hide_index=True)
                else:
                    # Display in two sections if both categories have documents
            ...
            # Discrepancies for this section
            section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "mortgage_documents"]
            if section_issues:
                st.subheader("Discrepancies")
                df_discrepancies = pd.DataFrame(section_issues)
                st.dataframe(df_discrepancies, use_container_width=True, hide_index=True)
        ...
        with st.expander("Initial Legal Opinion (Detailed View)", expanded=True):
            ...
            # Discrepancies for this section
            section_issues = [issue for issue in result.get("POTENTIAL ISSUES (FLAGS)", []) if isinstance(issue, dict) and issue.get("related_section") == "potential_issues"]
            if section_issues:
                st.subheader("Discrepancies")
                df_discrepancies = pd.DataFrame(section_issues)
                st.dataframe(df_discrepancies, use_container_width=True, hide_index=True)
        ...
        with st.expander("Debug: View Raw Data Structure", expanded=False):
            st.json(result)
...
def generate_legal_report_pdf(filename, analysis_data, rag_lookups=None):
    ...
    sections = ["property_address", "boundaries", "owner_names", "property_extent", 
                "title_history", "legal_opinion", "mortgage_documents"]
    
    # Check for issues in each section
    section_issues = {}
    for section in sections:
        issues = [i for i in analysis_data.get("potential_issues", []) 
                 if isinstance(i, dict) and i.get("related_section") == section]
        section_issues[section] = "⚠️" if issues else "✅"
    
    # Create status bar
    status_data = [[
        Paragraph(f"{section_issues['property_address']} Address", styles['SmallNormal']),
        Paragraph(f"{section_issues['boundaries']} Boundaries", styles['SmallNormal']),
        Paragraph(f"{section_issues['owner_names']} Owners", styles['SmallNormal']),
        Paragraph(f"{section_issues['property_extent']} Extent", styles['SmallNormal']),
        Paragraph(f"{section_issues['title_history']} Title", styles['SmallNormal']),
        Paragraph(f"{section_issues['legal_opinion']} Opinion", styles['SmallNormal']),
        Paragraph(f"{section_issues['mortgage_documents']} Mortgage", styles['SmallNormal'])
    ]]
    
    status_table = Table(status_data, colWidths=[1.2*inch] * 7)
    ...
    summary_data = [
        [Paragraph("<b>Address:</b>", styles['SmallNormal']), 
         Paragraph(str(analysis_data.get("property_address", "Not Found")), styles['SmallNormal'])],
        [Paragraph("<b>Owners:</b>", styles['SmallNormal']), 
         Paragraph(str(analysis_data.get("owner_names", "Not Found")), styles['SmallNormal'])],
        [Paragraph("<b>Extent:</b>", styles['SmallNormal']), 
         Paragraph(extent_str, styles['SmallNormal'])]
    ]
    
    # Add property nature if available
    ...
    # Mortgage Documents (simplified, side-by-side)
    ...
    if pre_disbursal or post_disbursal:
        left_column.append(Spacer(1, 5))
        left_column.append(Paragraph("Required Documents", styles['Heading2']))
        
        # If we only have pre-disbursal documents or no post-disbursal documents
        if pre_disbursal and not post_disbursal:
            docs_data = [[Paragraph("<b>Document Name</b>", styles['SmallNormal'])]]
            
            for doc in pre_disbursal:
                docs_data.append([Paragraph(str(doc.get("document_name", "")), styles['SmallNormal'])])
            ...
            left_column.append(docs_table)
        
        # If we have both pre and post-disbursal documents
        elif pre_disbursal and post_disbursal:
            # Create two columns for pre and post disbursal
            mortgage_data = [[
                Paragraph("<b>Pre-Disbursal Documents</b>", styles['SmallNormal']),
                Paragraph("<b>Post-Disbursal Documents</b>", styles['SmallNormal'])
            ]]
            
            # Determine the maximum number of documents
            max_docs = max(len(pre_disbursal), len(post_disbursal))
            
            for i in range(max_docs):
                pre_doc = pre_disbursal[i].get("document_name", "") if i < len(pre_disbursal) else ""
                post_doc = post_disbursal[i].get("document_name", "") if i < len(post_disbursal) else ""
                
                mortgage_data.append([
                    Paragraph(pre_doc, styles['SmallNormal']),
                    Paragraph(post_doc, styles['SmallNormal'])
                ])
            
            mortgage_table = Table(mortgage_data, colWidths=[2.5*inch, 2.5*inch])
            mortgage_table.setStyle(TableStyle([
                ('BACKGROUND', (0,0), (-1,0), colors.lightgrey),
                ('GRID', (0,0), (-1,-1), 0.5, colors.black),
                ('VALIGN', (0,0), (-1,-1), 'TOP')
            ]))
            left_column.append(mortgage_table)
        
        # If we only have post-disbursal documents
        elif not pre_disbursal and post_disbursal:
            docs_data = [[Paragraph("<b>Post-Disbursal Documents</b>", styles['SmallNormal'])]]
            
            for doc in post_disbursal:
                docs_data.append([Paragraph(str(doc.get("document_name", "")), styles['SmallNormal'])])
    ...
    if isinstance(opinion, dict):
        right_column.append(Paragraph("Legal Opinion", styles['Heading2']))
        
        # Compact opinion metrics
        metrics = {
        ...
        if "recommendations" in opinion:
            ...
            conclusion_data.append([
                Paragraph("<b>Recommendations:</b>", styles['SmallNormal']),
                Paragraph(recommendations[:200] + "..." if len(recommendations) > 200 else recommendations, 
                         styles['SmallNormal'])
            ])
    ...
    # Potential Issues
    issues = analysis_data.get("potential_issues", [])
    if isinstance(issues, list) and issues:
        right_column.append(Spacer(1, 5))
    ...
    # Documents Examined (Compact version)
    ...
    if isinstance(docs_examined, list) and docs_examined:
        ...
        docs_data = [[
            Paragraph("<b>Document</b>", styles['Tiny']),
            Paragraph("<b>Date</b>", styles['Tiny']),
            Paragraph("<b>Parties</b>", styles['Tiny'])
        ]]
        
        for doc in docs_examined:
            parties = f"{doc.get('executed_by', '')}"
            if doc.get('in_favour_of'):
                parties += f" → {doc.get('in_favour_of', '')}"
                
            row = [
                Paragraph(str(doc.get("document_name", ""))[:20], styles['Tiny']),
                Paragraph(str(doc.get("execution_date", ""))[:10], styles['Tiny']),
                Paragraph(parties[:30], styles['Tiny'])
            ]
            docs_data.append(row)
        
        docs_table = Table(docs_data, colWidths=[2*inch, 0.8*inch, 2.2*inch])
        docs_table.setStyle(TableStyle([
            ('BACKGROUND', (0,0), (-1,0), colors.lightgrey),
            ('GRID', (0,0), (-1,-1), 0.5, colors.black),
            ('VALIGN', (0,0), (-1,-1), 'TOP')
    ...
    if "final_grounded_opinion" in analysis_data and analysis_data["final_grounded_opinion"]:
        right_column.append(Spacer(1, 5))
        right_column.append(Paragraph("Final Grounded Opinion", styles['Heading2']))
        
        opinion_text = analysis_data["final_grounded_opinion"]
        right_column.append(Paragraph(opinion_text, styles['SmallNormal']))
    
    # RAG Lookups (Very compact version - just 1-2 most relevant)
    if rag_lookups and len(rag_lookups) > 0:
        right_column.append(Spacer(1, 5))
        right_column.append(Paragraph("Key Guidance", styles['Heading2']))
        
        # Sort by similarity and take top 2
        top_lookups = sorted(rag_lookups, key=lambda x: x['lookup']['similarity'], reverse=True)[:2]
        
        for lookup in top_lookups:
            lookup_data = [
                [Paragraph(f"<b>Q:</b> {lookup['lookup']['query'][:50]}...", styles['Tiny'])],
                [Paragraph(f"<b>A:</b> {lookup['lookup']['answer'][:100]}...", styles['Tiny'])]
            ]
            lookup_table = Table(lookup_data, colWidths=[5*inch])
            lookup_table.setStyle(TableStyle([
                ('GRID', (0,0), (-1,-1), 0.5, colors.lightgrey),
                ('VALIGN', (0,0), (-1,-1), 'TOP')
            ]))
            right_column.append(lookup_table)
            right_column.append(Spacer(1, 3))
    
    # Now create the two-column layout
    ...
    main_table = Table(data, colWidths=[5*inch, 5*inch])
    ...
    try:
        ...
        # If the two-column layout fails, fall back to a single-column layout
        print(f"Two-column layout failed: {e}. Falling back to single-column layout.")
        buffer = io.BytesIO()
        doc_template = SimpleDocTemplate(buffer, pagesize=landscape(letter), title=f"Legal Analysis: {filename}",
                                leftMargin=0.25*inch, rightMargin=0.25*inch,
                                topMargin=0.25*inch, bottomMargin=0.25*inch)
        
        # Combine all elements in a single column
        single_column = []
        single_column.append(Paragraph(f"LEGAL ANALYSIS REPORT: {filename}", styles['Title']))
        single_column.append(status_table)
        single_column.append(Spacer(1, 5))
        single_column.extend(left_column)
        single_column.extend(right_column)
        
        doc_template.build(single_column)
        return buffer.getvalue()
